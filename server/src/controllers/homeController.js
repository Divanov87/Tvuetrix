...
router.get('/sitemap.xml', async (req, res) => {
    try {
        // Set headers for XML and GZIP content
        ...
        // Get the sitemap (either cached or generated)
        const sitemap = await sitemapService.getSitemap(origin);

        // Send the sitemap response
        res.send(sitemap);
    } catch (error) {
...
router.get('/', async (req, res) => {
    const latestBlogs = await blogService.getPinned();
    try {
        ...
        if (city === undefined || city === null) {
            ...
            return res.status(200).json({ latestPins, topRatedEvents, theaterEvents, concertEvents, latestBlogs });
        }
        ...
        res.status(200).json({ latestPins, topRatedEvents, theaterEvents, concertEvents, city, latestBlogs });
    } catch (error) {
        console.error('Error in home route:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
